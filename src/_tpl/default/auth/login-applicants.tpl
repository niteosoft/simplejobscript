		...
		<div class="container">
			<div class="row">
				<div class="col-md-12 col-xs-12">
					...
					<h2>{$translations.login.headline_login} {$translations.login.submessage_applicant}</h2>
					...
					<form id="login-form" name="login-form" method="post" action="{$BASE_URL}login-applicant/" role="form">
					...
					{if $PROFILES_PLUGIN == 'true'}
						<a id="sign-up-link" href="{$BASE_URL}{$URL_REGISTER_APPLICANTS}" class="make-account">{$translations.login.noacc_applicant}</a>
					{/if}
					...
					<div id="forgotten-zone" class="displayNone">
						...
						<form role="form" action="" method="post" >
								...
								<button type="button" class="btn mbtn" onclick="return SimpleJobScript.passwordRecoveryApplicants();">{$translations.login.forgotten_submit}</button>
