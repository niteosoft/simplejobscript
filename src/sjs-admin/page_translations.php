...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && key_exists('action', $_POST)) 
{
	...
	switch ($_POST['action']) 
	{
		case 'add_language':
		{
			...
			{
				$cache->removeCache(CACHE_TRANSLATIONS.'_'.$code);clear_tpl_cache();
				echo 1;
			}
			else
			{
...
if (isset($_app_info['params'][1]) && $_app_info['params'][1] == 'edit')
{
	...
	$translator_edit = $translator_edit->getLanguageIdByCode();
	$smarty->assign('current_lang_id', $translator_edit['id']);
