...
class SimpleJobscriptSettings
{
	...
	public function GetSettingsCategories()
	{
		...
		while ($row = $result->fetch_assoc())
		{
			if (!defined('PAYPAL_PLUGIN') && ($row['var_name'] == "invoicing" or $row['var_name'] == "payment_settings"))
				continue;
	...
	public function UpdateSettings($settings_array)
	{
		...
		while($i < $count)
		{
			...
			if ($name === "job_expires"){
				...
				// in packages mode this field cannot be changed. Otherwise set it up
				if (PAYMENT_MODE == "0" || PAYMENT_MODE == "1" || PAYMENT_MODE == "2") {
					$quickSQL = 'UPDATE employer SET job_period = ' . intval($value);
					$db->query($quickSQL);
				}
