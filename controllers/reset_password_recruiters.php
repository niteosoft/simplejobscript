<?php
	...
	if (isset($_POST['forget_pass'])){
		...
		if ($result) {
			...
			$update_msg =  str_replace("{signin}", "<a class=\"themeLink\" href=\"/" . URL_LOGIN_RECRUITERS ."\">" . $translations['login']['login_submit'] . "</a>", $translations['login']['update_win']);
		...
		$result = $class->checkForPassRecoveryHash($id); //id=hash
		$userid = $class->getUserId();

		$useremail = $class->getUserEmail($userid);
		if (!empty($useremail))
	...
	$smarty->assign('FORM_URL', URL_RESET_PASSWORD_RECRUITERS);
	$template = 'auth/reset-password.tpl';
?>